...
public class SearchController {
    ...
    @RequestMapping("page")
    public PageResult<Goods> page(@RequestBody SearchRequst searchRequst) {
        System.out.println(searchRequst.getKey() + "---" + searchRequst.getPage());
        NativeSearchQueryBuilder builder = new NativeSearchQueryBuilder();

        builder.withQuery(QueryBuilders.matchQuery("all", searchRequst.getKey()).operator(Operator.AND));

        //过滤filter查询条件
        //匹配规格参数时改造
        BoolQueryBuilder builder1 = QueryBuilders.boolQuery().must(QueryBuilders.matchQuery("all",searchRequst.getKey()).operator(Operator.AND));
        if(searchRequst.getFilter()!=null && searchRequst.getFilter().size()>0){
            searchRequst.getFilter().keySet().forEach(key->{
                ...
                builder1.filter(QueryBuilders.termQuery(field,searchRequst.getFilter().get(key)));
            });
        }
        ...
        //分页
        builder.withPageable(PageRequest.of(searchRequst.getPage() - 1, searchRequst.getSize()));
        //排序
        builder.withSort(SortBuilders.fieldSort(searchRequst.getSortBy()).order(searchRequst.getDescending() ? SortOrder.DESC : SortOrder.ASC));
        ...
        if (categoryList.size() == 1) {
            //1:根据三级分类id查询规格参数
            List<SpecParam> specParams = specClient.findParamByCidAndSearch(categoryList.get(0).getId());
            ...
            return new SearchResult(search.getTotalElements(), search.getContent(), search.getTotalPages(), brandList, categoryList,paramList);
    }
